...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached:  # 序列长度增加
            inv_freq, self.attention_scaling = self.rope_init_fn(
                None, device, seq_len=seq_len, **self.rope_kwargs
            )
            self.register_buffer("inv_freq", inv_freq, persistent=False)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        key_states = (
            ...
            .view(bsz, seq_len, self.num_key_value_heads, self.head_dim)
            .transpose(1, 2)
        )
        value_states = (
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _update_causal_mask(
        self, attention_mask, input_tensor, cache_position, past_key_values
    ):
        past_seen_tokens = (
...
def temperature_sampling_generate(
    model, input_ids, max_new_tokens=10, temperature=1.0, attention_mask=None
):
...
class Engine:
    ...
    def execute(
        ...
        prompt = prompts[0]
        prompt_ids = self.tokenizer(prompt, return_tensors="pt")["input_ids"].view(
            1, -1
        )  # type:ignore
        output_ids = temperature_sampling_generate(
